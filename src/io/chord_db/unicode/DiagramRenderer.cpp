...
#include <musical/io/chord_db/unicode/DottedGrid.h>

#include <musical/io/chord_db/DiagramIO.h>
    ...
    const std::string&name)
{
    ...
    if(diagram.is_open())
    {
        ...
        grid->core().write_caged_row(::chord::db::to_string(diagram.caged()));        
    ...
    else
    {
        ...
        fill_grid(grid,diagram);   
        //hack: on ajoute une lignes vide pour compenser l’absence 
        //de la barre de nut.
        grid.pad_top(1);                        
        grid.write_caged_row(::chord::db::to_string(diagram.caged()));
...
void DiagramRenderer::fill_grid(
    ...
    const ::chord::db::Diagram& diagram)
{
    using Diagram = ::chord::db::Diagram;
